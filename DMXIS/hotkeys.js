document.addEventListener('keydown', function(event) {
    ...
    if (isColorMenu || isSearchMenu) {
        ...
        // Allow normal typing in color modal input
    ...
    // 'c': Open color menu
    if (event.key === 'c' && !isValueInput && !isSlider && !isColorMenu) {
        event.preventDefault();
        closeSearchModal();
        openColorModal();
        return;
    ...
    if (['ArrowLeft', 'ArrowRight'].includes(event.key) && !isValueInput && !event.metaKey && !isColorMenu && !isSearchMenu) {
        ...
        if (isFixtureLabelGrid || currentHighlightedFixture !== null) {
            ...
            if (selectedChannels.size === 0 && currentIndex === -1) {
                const targetContainer = event.key === 'ArrowRight' ? allContainers[0] : allContainers[allContainers.length - 1];
